...
def to_check_item(item):
    ...
    try:
        ...
        if not soup.find("div", class_="book__buy"):
            ...
            status_in_stock = soup.find("span", class_="instock1")
            if stock == 9999 or not status_in_stock:
                item["stock"] = "0"
            elif stock == 9999999:
                item["stock"] = "1"
            else:
...
async def get_compare():
    ...
    # Забираем книги, которые есть в других магазинах из файла (раскомментировать, если когда-то будут)
