...
ANTI_KEYWORDS = [
    ...
    "эколог",
    "сказки"
    # здравый контекст (часто ломает фильтр)
    "научн",
    "исследован",
    "исследования",
    "справочник",
    ...
    "сетев",
    "протокол",
    # популярные «ложные» слова
    "магнит",
    "магнитн",  # чтобы не ловить "магия" внутри
    "энергосбережение",
    "энергетическ систем",
]
pattern = re.compile(
    r"\b(" + "|".join(map(re.escape, KEYWORDS)) + r")\w*", re.IGNORECASE
)
anti_pattern = re.compile(
    r"\b(" + "|".join(map(re.escape, ANTI_KEYWORDS)) + r")\w*", re.IGNORECASE
)
