...
# --- Леммы (начальные формы) ключевых слов ---
# pymorphy сам приведёт "богом", "верой", "заветом" и т.д. к нормальной форме,
# поэтому не нужно вручную перечислять все словоформы
EXACT_LEMMAS = {
    "бог", "вера", "завет",
}

# Корни для поиска "начало слова" (устойчивые термины, где лемматизация
# не нужна или не помогает — иностранные слова, составные термины)
PREFIX_KEYWORDS = [
    # Религия
    "религ", "вероисповед", "верующ", "господ", "свят",
    "священ", "молитв", "молебен", "грехов", "исповед", "покаян",
    "проповед", "псалм", "христиан", "православ", "католич",
    "протестант", "мусульман", "коран", "хадис", "иудаизм", "еврейск",
    "тора", "талмуд", "буддизм", "буддист", "будда", "индуизм", "веды",
    "ведическ", "кришн", "шива",
    # Эзотерика / Гадания
    "таро", "гадани", "гадалк", "ясновид", "хироман",
]

# Слова, которые совпадают с "опасными" корнями, но чаще всего
# являются светскими и НЕ должны сами по себе считаться религиозным маркером
SAFE_WORDS_BY_ROOT = {
    "господ": ["господин", "госпожа", "господство", "господствовать", "господа"],
    "каран": ["карандаш", "карантин"],
    "расклад": ["расклад"],  # почти всегда светское слово ("расклад сил")
    "писани": ["писанина"],
}
...
prefix_pattern = re.compile(
    r"\b(" + "|".join(map(re.escape, PREFIX_KEYWORDS)) + r")\w*",
    re.IGNORECASE,
)

# Корни, для которых нет известных "безопасных" слов — совпадение по ним
# считается надёжным маркером религиозной тематики
_CONFIDENT_ROOTS = [r for r in PREFIX_KEYWORDS if r not in SAFE_WORDS_BY_ROOT]
confident_pattern = (
    re.compile(r"\b(" + "|".join(map(re.escape, _CONFIDENT_ROOTS)) + r")\w*", re.IGNORECASE)
    if _CONFIDENT_ROOTS
    else None
)
...
def _lemmas(text: str) -> set[str]:
    """Приводит все слова текста к начальной форме."""
    words = re.findall(r"[а-яёa-z]+", text.lower())
    return {morph.parse(w)[0].normal_form for w in words}


def _has_ambiguous_root_only(text: str) -> bool:
    """
    True, если все найденные "опасные" корни объясняются исключительно
    известными светскими словами, и при этом нет ни одного надёжного
    (confident) совпадения — то есть религиозного сигнала на самом деле нет.
    """
    matched_roots = {m.group(1).lower() for m in prefix_pattern.finditer(text)}
    ambiguous_roots = matched_roots & SAFE_WORDS_BY_ROOT.keys()
    if not ambiguous_roots:
        return False

    if confident_pattern and confident_pattern.search(text):
        return False

    for root in ambiguous_roots:
        found_words = re.findall(rf"\b{re.escape(root)}\w*", text, re.IGNORECASE)
        safe_words = SAFE_WORDS_BY_ROOT[root]
        # Если хоть одно совпавшее слово не входит в список безопасных —
        # это может быть настоящее религиозное слово (например "господь")
        if any(w.lower() not in safe_words for w in found_words):
            return False

    return True


def check_religions_book(title: str) -> bool:
    """
    Определяет по названию, относится ли книга к религиозно-эзотерической
    тематике, на основе точных лемм и ключевых корней.
    """
    text = title.lower()
    lemmas = _lemmas(text)

    # 1. Точное совпадение по леммам (бог/вера/завет в любой форме)
    if lemmas & EXACT_LEMMAS:
        return True

    # 2. Совпадение по корням
    if prefix_pattern.search(text):
        if _has_ambiguous_root_only(text):
            return False
        return True
...
def article_adapter(item_article: str) -> str:
    """
